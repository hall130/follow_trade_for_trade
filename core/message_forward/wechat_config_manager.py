...
class WeChatGroupConfigManager:
    """微信群配置管理器"""
    
    def __init__(self, config_file: str = 'wechat_group_config.json'):
        self.config_file = config_file
        self.wechat_platform = None
    ...
    async def initialize_wechat_platform(self) -> bool:
        """初始化微信平台"""
        try:
            wechat_config = {
                'enabled': True,
            ...
            self.wechat_platform = WeChatPlatform(wechat_config)
            
            # 连接到微信
            if await self.wechat_platform.connect():
                logger.info("✅ 微信平台连接成功")
                return True
            else:
                logger.error("❌ 微信平台连接失败")
                return False
    ...
    async def discover_groups(self) -> List[Dict[str, Any]]:
        """发现微信群"""
        if not self.wechat_platform or not self.wechat_platform.is_logged_in:
            logger.error("微信平台未连接")
            return []
        
        try:
            groups = self.wechat_platform.discover_groups()
    ...
    def generate_forward_rules(self, selected_groups: List[Dict[str, Any]]) -> List[ForwardRule]:
        ...
        for i, group in enumerate(selected_groups, 1):
            rule = ForwardRule(
                rule_id=f"wechat_rule_{i}",
                rule_name=f"微信群监听规则 {i}",
                enabled=True,
                source_platform="wechat",
                source_chat_ids=[group["id"]],
    ...
    async def cleanup(self):
        """清理资源"""
        if self.wechat_platform:
            await self.wechat_platform.disconnect()
...
async def interactive_config():
    ...
    manager = WeChatGroupConfigManager()
    
    try:
        # 初始化微信平台
        print("📱 正在连接微信...")
        if not await manager.initialize_wechat_platform():
            print("❌ 微信连接失败")
            return
        ...
        print("\n🎉 配置完成！")
        print("💡 现在可以运行消息转发管理器:")
        print("   python -m core.message_forward.manager")
