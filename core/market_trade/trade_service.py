...
from config.contract_config import get_contract_min_sz, get_contract_multiplier
from database.db import get_enabled_customers, get_enabled_signal_accounts
from utils.dingtalk_bot import send_trade_notification, send_alert_notification, send_alert_notification_async
from config.dingtalk_config import should_send_trade_notification, should_send_alert_notification, get_notification_at_settings
from datetime import datetime
...
class TradeService:
    ...
    def _ensure_dingtalk_initialized(self):
        """确保钉钉机器人已初始化"""
        try:
            from utils.dingtalk_bot import get_dingtalk_bot, init_dingtalk_bot
            ...
            if config and config.get("enabled", False):
                ...
                if webhook_url and webhook_url != "YOUR_ACCESS_TOKEN" and secret and secret != "YOUR_SECRET_KEY":
                    if init_dingtalk_bot(webhook_url, secret):
                        ...
                        logger.warning("⚠️ 交易服务钉钉机器人初始化失败")
                else:
    ...
    async def send_customer_trade_notification_async(self, customer_uid: str, symbol: str, direction: str, pos_side: str, 
        ...
        try:
            from utils.dingtalk_bot import get_dingtalk_bot, send_trade_notification_async
            from config.dingtalk_config import should_send_trade_notification
    ...
    def send_customer_trade_notification(self, customer_uid: str, symbol: str, direction: str, pos_side: str, 
        ...
        try:
            from utils.dingtalk_bot import get_dingtalk_bot, send_trade_notification
            from config.dingtalk_config import should_send_trade_notification
                ...
                for retry in range(max_retry):
                    ...
                    try:
                        ...
                        if res.get('code') == '0' and res.get('data') and res['data'][0].get('ordId'):
                            ...
                            from database.db import update_customer_trade_order_id, update_customer_trade_open_px
                            ...
                            if sCode == '51169' and reduceOnly:
                                ...
                                try:
                                    ...
                                    if trade_info:
                                        ...
                                        if current_closed == 0:
                                            from database.db import update_customer_trade_close_volume_contract
    ...
    async def batch_aggregate_place_orders(self, customer, symbol, direction, pos_side, signal_orders):
        ...
        try:
            ...
            if customer_data:
                ...
                from database.db import get_customer_effective_asset
            ...
            for order in signal_orders:
                ...
                try:
                    ...
                    # 获取信号源当前快照资产
                    from database.db import get_signal_source_current_asset
            ...
            if customer_trades_for_signal_order:
                ...
                if total_customer_reduce > 0:
                    ...
                    for customer_uid, customer_trades in customer_reduce_map.items():
                        # 计算该客户的总减仓量
                        customer_total_reduce = sum(float(get_trade_field(t, 'volume_contract') or 0) * signal_reduce_ratio for t in customer_trades)
                        customer_total_reduce = round(customer_total_reduce, get_contract_sz_precision(symbol))
    ...
    async def listen_customer_account(self, customer: Customer):
        from exchange.okx.okx_ws_client import OKXWebSocketClient
        client = await self.get_client(customer)
                ...
                if 'data' in data and isinstance(data['data'], list) and len(data['data']) > 0:
                    ...
                    if customer_data:
                        ...
                        if current_total_asset is None:
                            ...
                            if abs(asset - current_total_asset_float) / current_total_asset_float > 0.01:
                                # 如果资产变化超过1%，才更新
                                self.db_pool.execute(
                                    "UPDATE customers SET total_asset=%s WHERE customer_uid=%s AND is_demo=%s",
                                    (asset, customer_uid, is_demo)
    ...
    async def listen_customer_accounts(self):
        from database.db import get_enabled_customers
        is_demo = get_global_is_demo()
        ...
        try:
            ...
            if difference_sz > 0:  # 实际持仓过多，需要平仓
                ...
                if result and result.get('ordId'):
                    ...
                    # 插入customer_trades记录
                    from database.db import insert_customer_trade
                    insert_customer_trade(
                        self.db_pool,
                        customer_uid,
    ...
    async def _get_signal_source_local_positions(self, source_uid):
        ...
        try:
            ...
            positions = db_pool.query(
                "SELECT trade_uid, symbol, pos_side, volume_contract, open_px FROM signal_account_trades WHERE signal_source_uid=%s AND status='open'",
                (source_uid,)
            )
        ...
        try:
            ...
            for customer in customers:
                try:
                    ...
                    customer_uid = customer.get('customer_uid', 'unknown')
    ...
    async def _check_single_customer_stop_loss(self, customer):
        ...
        try:
            ...
            init_assets = float(customer.get('init_asset', 0))
            stop_loss_percent = float(customer.get('stop_loss_percent', 10.0))
            ...
            # 检查是否触发止损
            if actual_total_assets < stop_loss_line:
                logger.warning(f"[客户止损] 客户 {customer_uid} 触发止损！当前资产 {actual_total_assets:.2f} < 止损线 {stop_loss_line:.2f}")
    ...
    async def _get_customer_current_assets(self, customer_uid):
        ...
        try:
            ...
            if customer_data:
                ...
                init_asset = customer.get('init_asset')
    ...
    async def _sync_signal_source_assets_after_stop_loss(self, source_uid, current_assets):
        ...
        try:
            ...
            is_demo = get_global_is_demo()
            signal_data = self.db_pool.query(
                "SELECT api_key, secret_key, passphrase FROM signal_sources WHERE source_uid=%s AND is_demo=%s",
                (source_uid, is_demo)
            )
            ...
            response = await rest_client.get_account_balance()
            ...
            if response and response.get('code') == '0':
                ...
                if usdt_total > 0:
                    # 更新或插入USDT总资产记录
                    self.db_pool.execute(
                        "INSERT INTO signal_account_assets (signal_source_uid, asset) "
                        "VALUES (%s, %s) "
                        "ON DUPLICATE KEY UPDATE "
                        "asset = VALUES(asset), "
                        "snapshot_time = NOW()",
                        (source_uid, usdt_total)
                    )
    ...
    async def trigger_limit_follow_orders(self, signal_source_uid, symbol, pos_side, signal_price, signal_trade_uid):
        """触发限价跟单订单"""
        try:
            logger.info(f"[限价跟单] 触发限价跟单: {signal_source_uid} {symbol} {pos_side} @ {signal_price}")
            
            # 调用API接口执行限价跟单
            import aiohttp
            import json
            
            api_url = f"http://localhost:5000/api/v1/limit-follow/execute"
            payload = {
                'signal_trade_uid': signal_trade_uid,
                'signal_source_uid': signal_source_uid,
                'symbol': symbol,
                'pos_side': pos_side,
                ...
                async with session.post(api_url, json=payload) as response:
                    if response.status == 200:
                        ...
                        if result.get('success') == 200:
                            logger.info(f"[限价跟单] 执行成功: {result.get('message')}")
                        else:
                            logger.warning(f"[限价跟单] 执行失败: {result.get('message')}")
                    else:
    ...
    async def cancel_limit_follow_orders_on_signal_close(self, signal_source_uid, symbol, pos_side, signal_trade_uid):
        """信号源平仓时取消相关限价跟单订单"""
        try:
            logger.info(f"[限价跟单撤单] 信号源平仓，取消限价跟单: {signal_source_uid} {symbol} {pos_side}")
    ...
    async def _find_reduced_positions(self, exchange_positions, local_positions):
        ...
        try:
            ...
            for key, local_info in local_total_by_symbol.items():
                ...
                local_total_size = local_info['total_size']
    ...
    async def _get_signal_source_exchange_assets(self, signal_source):
        ...
        try:
            ...
            if hasattr(signal_source, 'api_key'):
                api_key = signal_source.api_key
                secret_key = signal_source.secret_key
                passphrase = signal_source.passphrase
                is_demo = signal_source.is_demo
            else:
                api_key = signal_source.get('api_key')
                secret_key = signal_source.get('secret_key')
    ...
    async def _calculate_customer_compensation_size(self, customer, signal_source, symbol, signal_size, signal_price):
        ...
        try:
            ...
            # 1. 获取客户有效资产
            from database.db import get_customer_by_id, get_customer_effective_asset
            is_demo = get_global_is_demo()
            customer_data = get_customer_by_id(self.db_pool, customer_uid, is_demo)
